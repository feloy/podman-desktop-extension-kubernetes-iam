...
import type { RpcChannel } from '@kubernetes-iam/rpc';
 ...
 *
 *  test('test', () => {
 *    vi.mocked(remoteMocks.get(API_IAM).createUser).mockResolvedValue(undefined);
 *    ...
 *    expect(remoteMocks.get(API_IAM).createUser).toHaveBeenCalled();
 *  });
 * ```
 */
export class RemoteMocks {
  ...
  reset(): void {
    ...
    vi.mocked(remoteMock.getProxy).mockImplementation(obj => {
      assert(this.#mocks.has(obj), `${String(obj)} is not mocked`);
