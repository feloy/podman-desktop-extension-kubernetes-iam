...
test('getSubscriptions returns unique values', async () => {
  const channel = 'channel1';
  const otherChannel = 'channel2';
  const subscriber = new ChannelSubscriber();
  await subscriber.subscribeToChannel(
    channel,
    {
      opt: 'value1',
    },
  ...
  expect(subscriber.getSubscriptions(channel)).toEqual([
    {
      opt: 'value1',
    },
    {
